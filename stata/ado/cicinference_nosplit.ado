program define cicinference_nosplit, rclass
    version 16.0

    /*
    Temporary no-split wrapper for migration tests only.
    This is not the final user-facing cicinference command.
    The required Mata files must be loaded before calling this program:
    ...
    */
    ...
    if (r(N) < 4) {
        display as error "no-split requires at least 4 complete observations"
        exit 2001
    }
